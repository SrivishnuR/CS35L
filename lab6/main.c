...
void* threadFunc(void* startIndex) {
  ...
  const double subsample_dy
    ...
    for( int px= start; px < width; px+=nthreads )
      {
...
int main( int argc, char **argv ) {
  ...
  if( nthreads < 1 ) {
    fprintf( stderr, "%s: Multithreading needs a value of 1 minimum.\n", argv[0] );
    return 1;
  }


  if( nthreads > width ) {
      printf("nthreads can not be greater than width.\n");
      nthreads = width;
  }
  ...
  int temp[width]; //All possible values that can be passed to threadFunc
  for (int i = 0; i < width; i++) {
    temp[i] = i;
  }

  for(int i = 0; i < nthreads; i++) {
    if(pthread_create(&tid[i], NULL, threadFunc, temp + i)) {
